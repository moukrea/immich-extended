...
- **Batching**: 250 assets/page (Immich cap, already wired). `MAX_PAGES_PER_SWEEP` bounds a single sweep so an initial 10k-asset backfill spreads over a few sweeps instead of pinning one tick. Suggested 8 pages (= 2 000 assets) per user per sweep; the watermark advances within the sweep so the next sweep resumes mid-backfill. (Same "bounded work per tick" principle as `engine_cycle::MAX_PAGES_PER_TICK`.)
