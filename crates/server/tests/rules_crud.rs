...
async fn poll_interval_defaults_to_300_when_absent() {
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let detail = body_json(resp).await;
    // The column is a fixed retained value now (cycle-6 L1: per-rule poll timers
    // retired, matching is event-driven). Create always writes 300; the field is
    // no longer operator-settable via the request body.
